"""GAG Team: API-ключ и профиль (Норвегия / Finn.no)."""
...
GAG_API_DOCS = (
    ...
    "🧩 Команда <b>GAG</b> · 🇳🇴 Норвегия (Finn.no)"
)
...
async def gag_service_menu(callback: CallbackQuery) -> None:
    ...
    cur = normalize_gag_service(
        (await get_setting(callback.from_user.id, GAG_SERVICE_KEY) or "").strip()
    ) or GAG_DEFAULT_SERVICE
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🟩 Finn.no",
                    callback_data=f"gag_service_set:{GAG_DEFAULT_SERVICE}",
                )
            ],
    ...
    await cq_edit_text(
        callback,
        "🧭 <b>Сервис GAG</b>\n\n"
        f"Код для <code>service</code> в API: <code>{e(cur)}</code>\n"
        "<i>Если у GAG другой код — напиши, подставим в <code>GAG_DEFAULT_SERVICE</code>.</i>",
        reply_markup=kb,
    )
    await callback.answer()
